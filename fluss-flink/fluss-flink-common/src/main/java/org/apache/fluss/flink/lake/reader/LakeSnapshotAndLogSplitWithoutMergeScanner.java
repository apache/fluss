...
/** A scanner to union all the lakehouse's snapshot and change log. */
public class LakeSnapshotAndLogSplitWithoutMergeScanner implements BatchScanner {
    ...
    public LakeSnapshotAndLogSplitWithoutMergeScanner(
            Table table,
    ...
    public CloseableIterator<InternalRow> pollBatch(Duration timeout) throws IOException {
        ...
        UnionCloseableIterator unionCloseableIterator =
                new UnionCloseableIterator(lakeRecordIterators);
    ...
    private static class UnionCloseableIterator implements CloseableIterator<InternalRow> {
        ...
        public UnionCloseableIterator(List<CloseableIterator<LogRecord>> logRecordIterators) {
            this.logRecordIterators = logRecordIterators;
            this.currentIteratorIndex = 0;
        }

        private void moveToNextValidIterator() {
            while (currentIteratorIndex < logRecordIterators.size()
        ...
        @Override
        public void close() {
            logRecordIterators.forEach(CloseableIterator::close);
        }

        @Override
        public boolean hasNext() {
            return currentIteratorIndex < logRecordIterators.size()
                    && logRecordIterators.get(currentIteratorIndex).hasNext();
        }
        ...
        public InternalRow next() {
            ...
            CloseableIterator<LogRecord> currentIterator =
                    logRecordIterators.get(currentIteratorIndex);

            if (!currentIterator.hasNext()) {
                moveToNextValidIterator();
            }
            InternalRow row = currentIterator.next().getRow();
            return row;
        }
    }
