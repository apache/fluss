    ...
    public ScanRuntimeProvider getScanRuntimeProvider(ScanContext scanContext) {
        ...
        if (enableChangelog) {
            if (selectedMetadataFields == null) {
                // Default behavior based on projected fields
                this.selectedMetadataFields =
                        (projectedFields == null || projectedFields.length == 0)
                                ? new int[] {
                                    0, 1, 2
                                } // SELECT * or metadata-only: include all metadata
                                : new int[0]; // Physical fields only: no metadata
            }
            ...
            if (!projectedFieldNames.isEmpty()) {
                ...
                // Metadata-only query: not supported
                throw new UnsupportedOperationException(
                        "Queries selecting only metadata columns are not supported. "
                                ...
                                + "Use:\n"
                                + "    SELECT _change_type, _log_offset, _commit_timestamp, physicalField FROM changelog_test$changelog");
            }
        } else if (projectedFields != null) {
            // Non-changelog tables: use standard index-based projection
