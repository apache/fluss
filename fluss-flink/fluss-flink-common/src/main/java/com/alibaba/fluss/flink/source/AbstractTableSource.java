...
public abstract class AbstractTableSource
    ...
    protected final org.apache.flink.table.types.logical.RowType tableOutputType;
    // will be empty if no primary key
    protected final int[] primaryKeyIndexes;
    // will be empty if no bucket key
    protected final int[] bucketKeyIndexes;
    // will be empty if no partition key
    protected final int[] partitionKeyIndexes;
    protected final boolean streaming;
    protected final FlinkConnectorOptionsUtils.StartupOptions startupOptions;

    // options for lookup source
    protected final int lookupMaxRetryTimes;
    protected final boolean lookupAsync;
    @Nullable protected final LookupCache cache;

    protected final long scanPartitionDiscoveryIntervalMs;
    protected final boolean isDataLakeEnabled;
    @Nullable protected final MergeEngineType mergeEngineType;

    // output type after projection pushdown
    protected LogicalType producedDataType;

    // projection push down
    @Nullable protected int[] projectedFields;

    @Nullable protected GenericRowData singleRowFilter;

    // whether the scan is for row-level modification
    @Nullable protected RowLevelModificationType modificationScanType;
    ...
    public abstract Source<RowData, ?, ?> source(
            ...
            long scanPartitionDiscoveryIntervalMs,
            FlussDeserializationSchema<RowData> deserializationSchema,
            boolean streaming);
