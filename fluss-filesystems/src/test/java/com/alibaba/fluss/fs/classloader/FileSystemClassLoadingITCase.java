...
package com.alibaba.fluss.fs.classloader;
...
import com.alibaba.fluss.fs.plugin.FileSystemPluginLoadingITCase;
...
/**
 * Integration test for filesystem class loading scenarios.
 *
 * <p>This test verifies:
 * 1. Multiple filesystem plugins loaded simultaneously (OSS + S3 + HDFS)
 * 2. Plugin class isolation verification
 * 3. Plugin dynamic unload and reload
 * 4. Class conflict detection
 */
public class FileSystemClassLoadingITCase {
    ...
    void testPluginClassIsolation() throws Exception {
        ...
        // Test with isolated class loaders
        URLClassLoader hdfsClassLoader = ClassLoaderTestUtils.createIsolatedClassLoader(
                tempDir.toString());
        URLClassLoader ossClassLoader = ClassLoaderTestUtils.createIsolatedClassLoader(
                tempDir.toString());
        URLClassLoader s3ClassLoader = ClassLoaderTestUtils.createIsolatedClassLoader(
                tempDir.toString());
        
        // Verify isolation
        assertThat(hdfsClassLoader).isNotEqualTo(ossClassLoader);
        assertThat(hdfsClassLoader).isNotEqualTo(s3ClassLoader);
        assertThat(ossClassLoader).isNotEqualTo(s3ClassLoader);
        
        // Clean up
        hdfsClassLoader.close();
        ossClassLoader.close();
        s3ClassLoader.close();
    }
    ...
    void testClassConflictDetection() throws Exception {
        ...
        // Test version conflict simulation
        ...
        assertThat(version1ClassLoader).isNotEqualTo(version2ClassLoader);
        
        // Clean up
        version1ClassLoader.close();
        version2ClassLoader.close();
    }
