...
But before that, let's start the [Tiering Service](https://fluss.apache.org/docs/maintenance/tiered-storage/lakehouse-storage/#start-the-datalake-tiering-service) that allows offloading the tables as `Lakehouse` tables.
...
When all your sources share a primary key - otherwise you can mix & match [streaming lookup joins](https://fluss.apache.org/docs/engine-flink/lookups/) - you can turn the problem on its head: update a unified table incrementally, rather than joining streams on the fly. 
