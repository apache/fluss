...
checkout is still at the recorded RC commit and clean, and rerun
`mvn -B --no-transfer-progress clean package -pl fluss-dist -am -DskipTests`;
if it still fails, fix the packaging and create a new RC instead of copying the
files manually; only after the verification succeeds should you prepare the
Docker build context and build and push the image:
