...
Before tagging, complete the [client version and dependency audit preparation](#9-publish-client-rc-artifacts-cratesio--pypi)
from the repository root and commit any resulting changes, including both Rust
workspaces' lockfiles. Return to `tools/`, then tag the release commit and export
its full hash for all subsequent builds:
...
```bash
test "$(git rev-parse HEAD)" = "${RELEASE_COMMIT:?Set the recorded RC commit}" || exit 1
