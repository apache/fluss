...
## COMPACTED Table Format

The COMPACTED table format is designed for efficient key-value workloads
where only the latest value for each key needs to be retained.

### Supported Table Types

- Log Tables
- Primary Key (PK) / Key-Value (KV) Tables

### How It Works

When using the COMPACTED format, Fluss keeps only the most recent value
for each key. Older values are removed during compaction, reducing
storage usage and processing overhead.
...
- Lightweight KV stores
- System or metadata tables
- Scenarios that do not require full changelog reads or column projections

### Limitations

- Not suitable for workloads requiring full changelog history
- Not recommended when column-level projections are needed
