...
default is 7 days. A non-positive value disables TTL cleanup.
...
`table.log.local-ttl` controls how long local log segments are retained. Its default is 1 day. An
explicitly configured value must be greater than zero and, when `table.log.ttl` is positive, less
...
table, it might remain active after the table log TTL expires. The server option
`log.retention.roll-active-segment.enabled` controls whether Fluss rolls such a segment. It is
disabled by default.

When the option is enabled, Fluss can roll a non-empty active segment after `table.log.ttl` expires,
provided that the high watermark has reached the log end offset. The resulting inactive segment can
then be uploaded to remote storage and, after satisfying `table.log.local-ttl`, removed from local
storage.

The complete lifecycle is:

1. Records are appended to the active local segment.
2. After the table log TTL expires and all records are committed, active-segment rolling makes the
   segment inactive.
3. The inactive segment is copied to remote storage.
4. The copied local segment becomes eligible for cleanup according to `table.log.local-ttl`.
