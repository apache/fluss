...
### Handling out-of-order updates with sequence groups

Partial update keeps the last written value of each column, whether or not it is actually the newest record. When
several writers update the same row — a payment stream and a shipping stream, say — an out-of-order write silently
overwrites values that are already newer.
...
within a single write one group may move forward while another does not.

#### Define a sequence group
...
- A sequence column is arbitrated by the very group it orders, keeping its value in step with the columns it
  protects.
...
#### Supported merge engines

Sequence groups are supported by the [Default Merge Engine](../merge-engines/default.md) and the
[Aggregation Merge Engine](../merge-engines/aggregation.md); the pages describe what a group means under each engine.
A full-row write is arbitrated the same way as a partial one.
