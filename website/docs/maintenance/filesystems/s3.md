...
To enable S3 as remote storage, there are some additional configurations that must be set in Fluss, depending on the authentication method (credential provider), also in the Flink JobManager configuration. 
...
:::warning
This method in general also works when all Fluss components are running on AWS. However, you should prefer [AWS Credential Providers](#aws-credential-providers), as they also avoid embedding long-term access keys and secrets on the server.
:::

The token delegation process avoids embedding long-term credentials on the _client side_. 
Instead, Fluss clients continuously retrieve temporary credentials from the servers (token delegation). 
...
:::note
In Fluss the default credential provider chain (`s3.aws.credentials.provider`) does not contain any credential providers.
Credential providers must always be set explicitly.
:::

If you are not using the Fluss credential provider, each Fluss component authenticates itself with its own set of credentials.
...
When all Fluss components are running on AWS, you can rely on [AWS's credential providers](https://hadoop.apache.org/docs/r3.3.4/hadoop-aws/tools/hadoop-aws/index.html#auth_providers).
...
```yaml
# Depending on the credential provider, additional configuration might be needed (see below)
fluss.client.fs.s3.aws.credentials.provider: <AWS credential provider(s)>
```

:::warning
We highly recommend to use AWS credential providers that use _temporary credentials_ (e.g., `org.apache.hadoop.fs.s3a.auth.IAMInstanceCredentialsProvider`).
:::
...
```yaml
fluss.client.fs.s3.aws.credentials.provider: org.apache.hadoop.fs.s3a.SimpleAWSCredentialsProvider
