...
* **setBounded(OffsetsInitializer stoppingOffsetsInitializer):** Makes the source bounded: it stops once it reaches the given stopping offsets and then finishes, even in streaming execution mode (see [Bounded Reads](#bounded-reads))
...
// Read up to the latest offsets captured at startup and then finish
FlussSource<Order> latestSource = FlussSource.<Order>builder()
    .setStartingOffsets(OffsetsInitializer.earliest())
