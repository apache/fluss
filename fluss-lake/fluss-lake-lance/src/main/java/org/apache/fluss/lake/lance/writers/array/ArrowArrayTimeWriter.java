...
/** {@link ArrowFieldWriter} for Time elements in arrays. */
public class ArrowArrayTimeWriter extends ArrowFieldWriter<InternalArray> {

    public static ArrowArrayTimeWriter forField(ValueVector valueVector) {
        return new ArrowArrayTimeWriter(valueVector);
    }

    private ArrowArrayTimeWriter(ValueVector valueVector) {
        super(valueVector);
        checkState(
    ...
    public void doWrite(InternalArray array, int ordinal, boolean handleSafe) {
        ...
        if (isNullAt(array, ordinal)) {
            ...
            int sec = readTime(array, ordinal) / 1000;
            if (handleSafe) {
            ...
            long microSec = readTime(array, ordinal) * 1000L;
            if (handleSafe) {
            ...
            long nanoSec = readTime(array, ordinal) * 1000000L;
            if (handleSafe) {
    ...
    private int readTime(InternalArray array, int ordinal) {
