...
class RemoteLogManifestJsonSerdeTest extends JsonSerdeTestBase<RemoteLogManifest> {
    ...
    @Test
    void testSerdeRoundTripAfterNewManifest() {
        // 1. Deserialize old-format JSON without remote_log_dir
        ...
        RemoteLogManifest oldManifest =
                RemoteLogManifestJsonSerde.fromJson(oldFormatJson.getBytes());
        assertThat(oldManifest.getRemoteLogDir()).isNull();

        // 2. Fix via newManifest()
        RemoteLogManifest fixedManifest = oldManifest.newManifest(REMOTE_LOG_DIR);
        assertThat(fixedManifest.getRemoteLogDir()).isEqualTo(REMOTE_LOG_DIR);

        // 3. Serialize the fixed manifest
        byte[] newFormatJson = fixedManifest.toJsonBytes();

        // 4. Deserialize the new-format JSON
        RemoteLogManifest roundTripManifest = RemoteLogManifestJsonSerde.fromJson(newFormatJson);

        // 5. Verify all fields survive the round trip
        assertThat(roundTripManifest.getRemoteLogDir()).isEqualTo(REMOTE_LOG_DIR);
        assertThat(roundTripManifest.getPhysicalTablePath()).isEqualTo(TABLE_PATH1);
        assertThat(roundTripManifest.getTableBucket()).isEqualTo(TABLE_BUCKET1);
        assertThat(roundTripManifest.getRemoteLogSegmentList()).hasSize(2);

        for (RemoteLogSegment segment : roundTripManifest.getRemoteLogSegmentList()) {
            assertThat(segment.remoteLogDir()).isEqualTo(REMOTE_LOG_DIR);
            assertThat(segment.physicalTablePath()).isEqualTo(TABLE_PATH1);
            assertThat(segment.tableBucket()).isEqualTo(TABLE_BUCKET1);
        }

        RemoteLogSegment seg1 = roundTripManifest.getRemoteLogSegmentList().get(0);
        assertThat(seg1.remoteLogSegmentId())
                .isEqualTo(UUID.fromString("a4421366-4a1d-4c3b-a0f8-0be2e77b1368"));
        assertThat(seg1.remoteLogStartOffset()).isEqualTo(0);
        assertThat(seg1.remoteLogEndOffset()).isEqualTo(9);
        assertThat(seg1.maxTimestamp()).isEqualTo(1722225103853L);
