...
public class DecimalSerializerTest extends SerializerTestBase<Decimal> {
    ...
    protected DecimalSerializer createSerializer() {
        return new DecimalSerializer(6, 3);
    }
    ...
    protected Decimal[] getTestData() {
        int precision = 6;
        int scale = 3;
        boolean isCompact = Decimal.isCompact(precision);

        // For compact precisions, use smaller values that fit in long
        return new Decimal[] {
            Decimal.fromUnscaledLong(1, precision, scale),
            Decimal.fromUnscaledLong(2, precision, scale),
            Decimal.fromUnscaledLong(3, precision, scale),
            Decimal.fromUnscaledLong(4, precision, scale),
            Decimal.fromUnscaledLong(0, precision, scale),
            Decimal.fromUnscaledLong(-12345, precision, scale),
            ...
            Decimal.fromBigDecimal(new BigDecimal("99.999"), precision, scale),
            Decimal.fromBigDecimal(new BigDecimal("-99.999"), precision, scale),
            Decimal.fromBigDecimal(new BigDecimal("0.001"), precision, scale),
            Decimal.fromBigDecimal(new BigDecimal("999.999"), precision, scale)
        };
    }
