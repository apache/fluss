...
class Months<T> extends TimeTransform<T> {
  ...
  public Integer apply(T t) {
    if (t instanceof TimestampNtz) {
      return ((TimestampNtz) t).toLocalDateTime().getMonth().getValue();
    ...
    throw new InvalidPartitionException(
        "source column must be date or timestamp for month transform");
  }
}
