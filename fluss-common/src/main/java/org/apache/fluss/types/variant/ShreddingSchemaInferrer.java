    ...
    public ShreddingSchema infer(
        ...
        for (FieldStatistics fieldStats : stats.values()) {
            ...
            if (presenceRatio >= presenceThreshold
                    ...
                    && dominantType != null
                    // TODO: FIP-36 follow-up: support nested candidates after shredding schema
                    // stores structured paths and Arrow writer/projection can prune nested values.
                    && ShreddedField.isTopLevelFieldPath(fieldStats.getFieldPath())
        ...
        for (int i = 0; i < candidates.size(); i++) {
            ...
            fields.add(new ShreddedField(candidate.fieldPath, dataType));
        }
    ...
    public Optional<ShreddingSchema> update(
        ...
        for (ShreddedField existing : current.getFields()) {
            ...
            if (stats != null) {
                ...
                if (presenceRatio >= presenceThreshold
                        && consistency >= typeConsistencyThreshold
                        && dominantType != null
                        && ShreddedField.isTopLevelFieldPath(existing.getFieldPath())
                        && SHREDDABLE_TYPES.contains(dominantType)
        ...
        for (FieldStatistics stats : newStats.values()) {
            ...
            if (presenceRatio >= presenceThreshold
                    && consistency >= typeConsistencyThreshold
                    && dominantType != null
                    && ShreddedField.isTopLevelFieldPath(stats.getFieldPath())
                    && SHREDDABLE_TYPES.contains(dominantType)) {
                newCandidates.add(
                        new CandidateField(
        ...
        if (!newCandidates.isEmpty()) {
            ...
            for (int i = 0; i < Math.min(newCandidates.size(), remaining); i++) {
                ...
                DataType dataType = mapToDataType(candidate.typeRoot);
                updatedFields.add(new ShreddedField(candidate.fieldPath, dataType));
            }
        }
