...
public final class VariantUtil {
    ...
    private static void objectToJson(byte[] metadata, byte[] value, int offset, StringBuilder sb) {
        ...
        for (int i = 0; i < numFields; i++) {
            ...
            int fieldId = readUnsignedLE(value, idStart + i * idSize, idSize);
            String fieldName = metadataFieldName(metadata, fieldId);
            sb.append('"');
            escapeJsonString(fieldName, sb);
            sb.append("\":");

            int fieldOffset = readUnsignedLE(value, offsetStart + i * offsetSize, offsetSize);
            toJson(metadata, value, dataStart + fieldOffset, sb);
        }
        sb.append('}');
    }
    ...
    private static void arrayToJson(byte[] metadata, byte[] value, int offset, StringBuilder sb) {
        ...
        for (int i = 0; i < numElements; i++) {
            ...
            int elementOffset = readUnsignedLE(value, offsetListStart + i * offsetSize, offsetSize);
            toJson(metadata, value, dataStart + elementOffset, sb);
        }
        sb.append(']');
    }
