...
public class Variant implements Serializable {
    ...
    public Variant getFieldByName(String fieldName) {
        byte[] m = metadata();
        byte[] v = value();
        int fieldId = VariantUtil.findFieldId(m, fieldName);
        if (fieldId < 0) {
            return null;
        }
        int valueOffset = VariantUtil.findFieldValueOffset(v, 0, fieldId);
        if (valueOffset < 0) {
            return null;
        }
        int valueSize = VariantUtil.valueSize(v, valueOffset);
        byte[] fieldValue = Arrays.copyOfRange(v, valueOffset, valueOffset + valueSize);
        return new Variant(m, fieldValue);
    }
    ...
    public Variant getElementAt(int index) {
        byte[] v = value();
        int elementOffset = VariantUtil.arrayElementOffset(v, 0, index);
        int elementSize = VariantUtil.valueSize(v, elementOffset);
        byte[] elementValue = Arrays.copyOfRange(v, elementOffset, elementOffset + elementSize);
