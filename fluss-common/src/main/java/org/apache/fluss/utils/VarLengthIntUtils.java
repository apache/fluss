...
public final class VarLengthIntUtils {
    ...
    public static int encodeLong(OutputView os, long value) throws IOException {
        ...
        int i = 1;
        while ((value & ~0x7FL) != 0) {
            os.writeByte((((int) value & 0x7F) | 0x80));
            value >>>= 7;
            i++;
        }
    ...
    public static long decodeLong(InputView is) throws IOException {

        long result = 0;
        for (int offset = 0; offset < 64; offset += 7) {
            long b = is.readByte() & 0xff;
            result |= (b & 0x7F) << offset;
            if ((b & 0x80) == 0) {
                return result;
            }
    ...
    public static int decodeInt(InputView is) throws IOException {
        for (int offset = 0, result = 0; offset < 32; offset += 7) {
            int b = is.readByte() & 0xff;
            result |= (b & 0x7F) << offset;
            if ((b & 0x80) == 0) {
                return result;
            }
