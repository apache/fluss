...
public class ArrowWriter implements AutoCloseable {
     ...
     * metadata length should be consistent if the arrow schema is not changed. When shredding is
     * active, this includes the Schema message size.
     */
    private final int metadataLength;

    /** Whether the writer uses variant shredding (typed_value children in the Arrow schema). */
    private final boolean hasShredding;

    /** The serialized size of the Arrow Schema IPC message (0 when no shredding). */
    private final int schemaMessageSize;
    ...
    ArrowWriter(
        ...
        this.schema = schema;
        this.root =
                shreddingSchemas != null && !shreddingSchemas.isEmpty()
                        ? VectorSchemaRoot.create(
                                ArrowUtils.toArrowSchemaWithShredding(schema, shreddingSchemas),
                                allocator)
                        : VectorSchemaRoot.create(ArrowUtils.toArrowSchema(schema), allocator);
        ...
        this.hasShredding = shreddingSchemas != null && !shreddingSchemas.isEmpty();
        if (hasShredding) {
            ByteArrayOutputStream baos = new ByteArrayOutputStream();
            try {
                MessageSerializer.serialize(
                        new WriteChannel(Channels.newChannel(baos)), root.getSchema());
            } catch (IOException e) {
                throw new RuntimeException("Failed to compute schema message size", e);
            }
            this.schemaMessageSize = baos.size();
        } else {
            this.schemaMessageSize = 0;
        }
        ...
        // Determine user-visible field count (no more $ prefix columns to skip)
        int userFieldCount = schema.getFieldCount();
        ...
        for (int i = 0; i < schema.getFieldCount(); i++) {
            ...
            if (dataType instanceof VariantType
                ...
                // Variant column with shredding - StructVector contains typed_value
                fieldWriters[i] =
                        ArrowUtils.createArrowShreddedVariantWriter(
                                (StructVector) fieldVector, shreddingSchemas.get(fieldName));
            } else {
                fieldWriters[i] = ArrowUtils.createArrowFieldWriter(fieldVector, dataType);
            }
        }
    }
    ...
    public int serializeToOutputView(AbstractPagedOutputView outputView) throws IOException {
        ...
        try (ArrowRecordBatch arrowBatch =
            ...
            // Serialize Arrow Schema message when shredding is active.
            // This allows readers to discover the actual schema structure
            // (including typed_value children) from the IPC stream.
            if (hasShredding) {
                totalSize += (int) MessageSerializer.serialize(writeChannel, root.getSchema());
            }

            ArrowBlock block = MessageSerializer.serialize(writeChannel, arrowBatch);
            ...
            totalSize += (int) (block.getMetadataLength() + block.getBodyLength());
            return totalSize;
        }
    }
