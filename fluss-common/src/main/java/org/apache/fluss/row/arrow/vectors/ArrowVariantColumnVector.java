...
/**
 * Arrow column vector for Variant type. Reads {@link Variant} values from a {@link VarBinaryVector}
 * where each entry stores the Variant binary encoding (metadata + value).
 */
@Internal
public class ArrowVariantColumnVector implements VariantColumnVector {

    /** Container which is used to store the sequence of variant values of a column to read. */
    private final VarBinaryVector varBinaryVector;

    public ArrowVariantColumnVector(VarBinaryVector varBinaryVector) {
