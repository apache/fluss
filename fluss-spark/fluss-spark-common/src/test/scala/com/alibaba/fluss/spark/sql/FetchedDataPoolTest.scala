...
import com.alibaba.fluss.spark.{SparkConnectorOptions, TableBucketInfo}
import com.alibaba.fluss.utils.clock.ManualClock

import org.apache.spark.SparkConf
import org.apache.spark.sql.fluss.FetchedDataPool
import org.apache.spark.sql.fluss.FetchedDataPool.{CachedFetchedDataList, CacheKey}
...
class FetchedDataPoolTest extends SharedSparkSession with PrivateMethodTester {
  ...
  test("acquire fetched data from multiple keys") {
    ...
    val cacheKeys = (0 until 10).map {
      bucket => CacheKey(new TableBucketInfo(new TableBucket(1, bucket)))
    }
  ...
  test("continuous use of fetched data from single key") {
    ...
    val cacheKey = CacheKey(new TableBucketInfo(new TableBucket(1, 0)))

    val data = dataPool.acquire(cacheKey, 0)
    data.withNewPoll(testRecords(0, 5).listIterator, 0)
  ...
  test("evict idle fetched data") {
    ...
    conf.set(
      SparkConnectorOptions.FETCHED_DATA_CACHE_TIMEOUT.key(),
      minEvictableIdleTimeMillis.toString)
    conf.set(
      SparkConnectorOptions.FETCHED_DATA_CACHE_EVICTOR_THREAD_RUN_INTERVAL.key(),
    ...
    val cacheKeys = (0 until 10).map {
      partId => CacheKey(new TableBucketInfo(new TableBucket(1, partId)))
    }
    ...
    clock.advanceTime(Duration.ofMillis(600))
    ...
    dataList.takeRight(3).foreach {
      case (key, data) =>
        dataPool.release(key, data)
    }

    // add objects to be candidates for eviction
    clock.advanceTime(Duration.ofMinutes(minEvictableIdleTimeMillis + 100))
  ...
  test("invalidate key") {
    ...
    val cacheKey2 = CacheKey(new TableBucketInfo(new TableBucket(1, 1)))
  ...
  private def testRecords(startOffset: Long, count: Int): ju.List[Record] = {
    (0 until count)
      .map {
        offset => new ScanRecord(startOffset + offset, 0, ChangeType.INSERT, new GenericRow(1))
      }
      .toList
      .asJava
  }
