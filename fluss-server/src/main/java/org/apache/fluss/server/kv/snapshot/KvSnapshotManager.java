...
public class KvSnapshotManager implements Closeable {
    ...
    public void downloadSnapshot(CompletedSnapshotHandle completedSnapshotHandle) throws Exception {
        CompletedSnapshot completedSnapshot =
                snapshotContext.getCompletedSnapshotProvider(tableBucket, completedSnapshotHandle);
    ...
    private KvSnapshotHandle getIncrementalKvSnapshotHandle(
        ...
        LOG.info(
                "{} sst files To download, and {} sst files to delete.",
                incrementalSstFileHandles.size(),
                sstFilesToDelete.size());

        // set a zero incremental size as this only a temporary snapshot.
        return new KvSnapshotHandle(incrementalSstFileHandles, privateFileHandles, 0);
    }
