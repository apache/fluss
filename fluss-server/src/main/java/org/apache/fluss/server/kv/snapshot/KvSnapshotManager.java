...
public class KvSnapshotManager implements Closeable {
    ...
    /**
     * Highest snapshot id that has been successfully downloaded by either {@link
     * #downloadSnapshot(long)} or {@link #downloadLatestSnapshot()}. Used to make the notification
     * path idempotent so that retries / duplicate notifications from coordinator (e.g. across
     * coordinator failover) do not redo verify+move on the same SST set.
     */
    private long lastDownloadedSnapshotId = -1L;

    /**
     * Monotonically increasing epoch bumped on every role transition. Downloads capture the epoch
     * at their start and periodically compare against the current value; a mismatch indicates a
     * concurrent role change and the download aborts cooperatively. This prevents a timed-out
     * standby download from continuing to write into the {@code db/} directory while the new role's
     * code path (e.g. leader's loadKv or follower's deleteDirectory) operates on the same files.
     */
    ...
    private void clearStandbyDownloadCache() {
        ...
        // Reset idempotency cursor: when the replica leaves standby, the next standby
        // promotion may need to re-download a snapshot whose id is <= the previous one
        // (e.g. coordinator failover replays an older snapshot id). Forgetting the cursor
        // here ensures we don't accidentally short-circuit a legitimate fresh download.
    ...
    private Path getKvDbPath() {
        return tabletDir.toPath().resolve(RocksDBKvBuilder.DB_INSTANCE_DIR_STRING);
    }
    ...
    public void startPeriodicUploadSnapshot(
        ...
        if (snapshotIntervalSupplier.getAsLong() > 0) {
            ...
            // triggerAllNonPeriodicTasks() from entering an infinite loop when the stale
    ...
    public void downloadSnapshot(long snapshotId) throws Exception {
        ...
        if (snapshotId <= lastDownloadedSnapshotId) {
            LOG.debug(
                    "Skip downloading snapshot {} for bucket {} because a snapshot with id >= {} "
    ...
    public Optional<CompletedSnapshot> downloadLatestSnapshot() throws Exception {
        ...
        try {
            ...
            if (latestSnapshot.isPresent()) {
                ...
                standbySnapshotSize = completedSnapshot.getSnapshotSize();
                lastDownloadedSnapshotId =
                        Math.max(lastDownloadedSnapshotId, completedSnapshot.getSnapshotID());
                LOG.info(
    ...
    private void incrementalDownloadSnapshot(CompletedSnapshot completedSnapshot, int expectedEpoch)
            throws Exception {
        ...
        atomicDownloadSnapshot(incrementalSnapshot, sstFilesToDelete, expectedEpoch);
    ...
    /**
     * Check whether the download epoch has changed since the download started. If so, a role
     * transition has occurred and this download should abort to avoid corrupting the db/ directory
     * that the new role is about to use.
     */
    private void checkDownloadNotCancelled(int expectedEpoch) throws IOException {
        if (downloadEpoch.get() != expectedEpoch) {
            throw new IOException(
                    "Download cancelled due to role transition for bucket " + tableBucket);
        }
    }
    ...
    private void atomicDownloadSnapshot(
            CompletedSnapshot incrementalSnapshot, Set<Path> sstFilesToDelete, int expectedEpoch)
            throws IOException {
        ...
        try {
            ...
            if (tempDownloadDir.toFile().exists()) {
                try {
                    ...
                    LOG.debug(
                            "Cleaned up temporary snapshot directory {} for bucket {} (download {})",
                            tempDownloadDir,
                            tableBucket,
