...
public class ZooKeeperClient implements AutoCloseable {
    ...
    /**
     * Lists all completed bucket snapshots for the given {@link TableBucket}. Reads the {@code
     * BucketSnapshotsZNode} subtree as a single ZK getChildren followed by per-snapshot getData.
     * Snapshot ids are returned ordered ascending.
     */
    public List<BucketSnapshotIdAndData> listBucketSnapshots(TableBucket tableBucket)
            throws Exception {
        String path = BucketSnapshotsZNode.path(tableBucket);
        List<String> snapshotIdStrs = getChildren(path);
        List<BucketSnapshotIdAndData> result = new ArrayList<>(snapshotIdStrs.size());
        for (String snapshotIdStr : snapshotIdStrs) {
            long snapshotId = Long.parseLong(snapshotIdStr);
            String childPath = BucketSnapshotIdZNode.path(tableBucket, snapshotId);
    ...
    /** Tuple of (snapshotId, BucketSnapshot json data) for {@link #listBucketSnapshots}. */
    public static final class BucketSnapshotIdAndData {
        private final long snapshotId;
        private final BucketSnapshot bucketSnapshot;

        public BucketSnapshotIdAndData(long snapshotId, BucketSnapshot bucketSnapshot) {
