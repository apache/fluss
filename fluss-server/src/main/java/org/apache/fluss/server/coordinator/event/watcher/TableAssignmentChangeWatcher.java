...
public class TableAssignmentChangeWatcher {
    ...
    public static boolean validBucketChange(
        ...
        if (!newBucketAssignments.keySet().containsAll(oldBucketAssignments.keySet())
                || newBucketAssignments.size() <= oldBucketAssignments.size()) {
            return false;
        }

        // Verify that existing bucket assignments have not been modified
        for (Map.Entry<Integer, BucketAssignment> entry : oldBucketAssignments.entrySet()) {
            if (!entry.getValue().equals(newBucketAssignments.get(entry.getKey()))) {
                return false;
            }
        }
        return true;
