...
public class RemoteLogManager implements Closeable {
    ...
    public void startLogTiering(Replica replica) throws Exception {
        ...
        RemoteLogTablet remoteLog =
                new RemoteLogTablet(
                        physicalTablePath, tableBucket, replica.getLogTTLMs(), remoteLogDir);
        ...
        if (remoteLogManifestHandleOpt.isPresent()) {
            ...
            // If the RemoteLogManifest does not include remoteLogDir, it means the manifest was
            // generated by an old version that does not support remote.data.dirs.
            // We set remoteLogDir manually here, so subsequent usage will be safe to directly use
            // it.
            if (manifest.getRemoteLogDir() == null) {
                LOG.info(
                        "RemoteLogManifest loaded from old version without remoteLogDir, "
                                + "setting remoteLogDir to {} for bucket {}",
                        remoteLogDir,
                        manifest.getTableBucket());
                manifest = manifest.newManifest(remoteLogDir);
            }
    ...
    public void stopReplica(Replica replica, boolean deleteRemote) {
        ...
        if (deleteRemote) {
            ...
            deleteRemoteLog(remoteLogDir, physicalTablePath, tb);
        }
    }
    ...
    private void deleteRemoteLog(
        ...
        try {
            ...
            remoteLogStorage.deleteTableBucket(remoteLogDir, physicalTablePath, tableBucket);
