...
public final class KvTablet {
     ...
     * schema using column IDs. This is needed when a client sends a partial update with an older
     * schema and positions need to be remapped to the latest schema layout.
     */
    static int[] convertTargetColumns(int[] positions, Schema sourceSchema, Schema targetSchema) {
        ...
        for (int i = 0; i < positions.length; i++) {
            int colId = sourceColIds.get(positions[i]);
            Integer targetPos = targetIdToPos.get(colId);
            if (targetPos == null) {
                throw new IllegalArgumentException(
                        "Target column (id=" + colId + ") not found in latest schema");
            }
            result[i] = targetPos;
        }
        return result;
