 ...
 * 1. Multi-TabletServer class loading consistency
 * 2. CoordinatorServer restart class loading
 * 3. Rolling upgrade class loading
 */
public class ServerClassLoadingITCase {
    ...
    void testMultiTabletServerClassLoadingConsistency() throws Exception {
        ...
        URLClassLoader server1ClassLoader = ClassLoaderTestUtils.createIsolatedClassLoader(
                tempDir.toString());
        URLClassLoader server2ClassLoader = ClassLoaderTestUtils.createIsolatedClassLoader(
                tempDir.toString());
        URLClassLoader server3ClassLoader = ClassLoaderTestUtils.createIsolatedClassLoader(
                tempDir.toString());
        ...
        Class<?> coordinatorClass1 = server1ClassLoader.loadClass(CoordinatorServer.class.getName());
        Class<?> coordinatorClass2 = server2ClassLoader.loadClass(CoordinatorServer.class.getName());
        Class<?> coordinatorClass3 = server3ClassLoader.loadClass(CoordinatorServer.class.getName());
    ...
    void testCoordinatorServerRestartClassLoading() throws Exception {
        ...
        URLClassLoader restartClassLoader = ClassLoaderTestUtils.createIsolatedClassLoader(
                tempDir.toString());
        ...
        URLClassLoader afterRestartClassLoader = ClassLoaderTestUtils.createIsolatedClassLoader(
                tempDir.toString());
        
        // Load the same class after restart
        Class<?> afterRestartClass = afterRestartClassLoader.loadClass(TabletServer.class.getName());
    ...
    void testRollingUpgradeClassLoading() throws Exception {
        ...
        URLClassLoader oldVersionClassLoader = ClassLoaderTestUtils.createIsolatedClassLoader(
                tempDir.toString());
        URLClassLoader newVersionClassLoader = ClassLoaderTestUtils.createIsolatedClassLoader(
                tempDir.toString());
        ...
        Class<?> oldCoordinatorClass = oldVersionClassLoader.loadClass(CoordinatorServer.class.getName());
        Class<?> newCoordinatorClass = newVersionClassLoader.loadClass(CoordinatorServer.class.getName());
        ...
        URLClassLoader oldWithExclusions = ClassLoaderTestUtils.createClassLoaderWithExclusions(
                sharedPackages, tempDir.toString());
        URLClassLoader newWithExclusions = ClassLoaderTestUtils.createClassLoaderWithExclusions(
                sharedPackages, tempDir.toString());
