...
class FlussLakeTableITCase {
    ...
    /**
     * Test that for legacy tables (created before kv format version was introduced), the key
     * encoding still uses the Paimon encoder
     *
    ...
    void testLegacyPKTableUsePaimonEncoder() throws Exception {
        ...
        // Hack ZK node to remove kv format version to simulate legacy table
        removeKvFormatVersionFromZk(tablePath);
    ...
    /**
     * Removes the kv format version from the table registration in ZooKeeper to simulate a legacy
     * table created before the kv format version feature was added.
     */
    private void removeKvFormatVersionFromZk(TablePath tablePath) throws Exception {
        ZooKeeperClient zkClient = FLUSS_CLUSTER_EXTENSION.getZooKeeperClient();

        // Get current table registration
        TableRegistration tableReg = zkClient.getTable(tablePath).get();

        // Create new properties without kv format version
        Map<String, String> newProperties = new HashMap<>(tableReg.properties);
        newProperties.remove(ConfigOptions.TABLE_KV_FORMAT_VERSION.key());

        // Update table registration with new properties
        TableRegistration newTableReg =
                tableReg.newProperties(newProperties, tableReg.customProperties);
        zkClient.updateTable(tablePath, newTableReg);
    }
}
