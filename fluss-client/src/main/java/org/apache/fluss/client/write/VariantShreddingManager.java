 ...
 * {@link VariantStatisticsCollector}s. Once the minimum sample threshold is met, the inference
 * result is stored locally and made available via {@link #getShreddingSchemas()}.
 *
 * <p>Inference is triggered <em>at most once</em> per manager instance. Once inference completes,
 * even with an empty result, subsequent calls to {@link #collectRow} are no-ops.
 *
 * <p>Thread safety: the manager synchronizes statistics collection internally because a table-level
 * manager can be shared by appends to different bucket deques.
 */
@Internal
public class VariantShreddingManager {
    ...
    /**
     * Locally inferred shredding schemas. Key is the variant column name. Empty before inference
     * completes or when no eligible fields are found.
     */
     ...
     * collected, the inference result is stored locally.
     *
     * @param row the row being written
     */
    public synchronized void collectRow(InternalRow row) {
    ...
    private void maybeInfer() {
        ...
        if (!schemas.isEmpty()) {
            ...
            LOG.debug(
                    "Completed Variant shredding inference for table {} with no eligible fields.",
                    tablePath);
        }
    }
}
