...
public class FlussTypeUtils {
     ...
     * 
     * <p>This method handles the conversion from Fluss data types to Trino types.
     * It supports all basic types as well as complex types like arrays, maps, and rows.
     * For unsupported types, it falls back to VARCHAR.
     */
    public static Type toTrinoType(DataType flussType, TypeManager typeManager) {
        try {
            ...
            switch (typeRoot) {
                ...
                case ROW:
                    return convertRowType((RowType) flussType, typeManager);
                default:
                    log.warn("Unsupported Fluss type root: %s, using VARCHAR as fallback", typeRoot);
                    return VarcharType.VARCHAR;
            }
        } catch (Exception e) {
            log.error(e, "Error converting Fluss type %s to Trino type, falling back to VARCHAR", flussType);
            return VarcharType.VARCHAR;
        }
    }
    ...
    public static Optional<DataType> toFlussType(Type trinoType) {
        try {
            ...
            log.error(e, "Error converting Trino type %s to Fluss type", trinoType);
            return Optional.empty();
        }
    }
}
