 ...
 * 1. Multiple threads loading plugins simultaneously
 * 2. Concurrent ClassLoader initialization
...
public class ConcurrentClassLoaderITCase {
    ...
    void testMultipleThreadsLoadingPluginsSimultaneously() throws InterruptedException {
        ...
        for (int i = 0; i < classLoaders.length; i++) {
            classLoaders[i] = ClassLoaderTestUtils.createIsolatedClassLoader(
                    tempDir.toString());
        }
        
        // Test concurrent class loading
        Class<?>[] loadedClasses = ClassLoaderTestUtils.testConcurrentClassLoading(
    ...
    void testConcurrentClassLoaderInitialization() throws InterruptedException {
        ...
        for (int i = 0; i < classLoaders.length; i++) {
            ...
            threads[i] = new Thread(() -> {
                try {
                    classLoaders[index] = ClassLoaderTestUtils.createIsolatedClassLoader(
                            tempDir.toString());
                } catch (Exception e) {
                    exceptions[index] = e;
                }
    ...
    void testRaceConditionDetection() throws InterruptedException {
        ...
        URLClassLoader classLoader1 = ClassLoaderTestUtils.createIsolatedClassLoader(
                tempDir.toString());
        URLClassLoader classLoader2 = ClassLoaderTestUtils.createIsolatedClassLoader(
                tempDir.toString());
